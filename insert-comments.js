(async () => {
  ...
  const timestampRegex =
    /^(?:\d+(?::[0-5][0-9]:[0-5][0-9])?|[0-5]?[0-9]:[0-5][0-9])$/gm;
  ...
  (() => {
    ...
    let w = d * 7;
    let y = d * 365.25;
    window.ms = function (val, options) {
      options = options || {};
      let type = typeof val;
  ...
  const EXAMPLE_MESSAGE_PAYLOAD_GEN = () => {
    return {
      user_data: {
        avatar:
          "https://static.crunchyroll.com/assets/avatar/60x60/1044-jujutsu-kaisen-satoru-gojo.png",
        name: "Gojo",
      },
...
<div class="content-image--3na7E content-image--is-background-type-one--1LQDe avatar__image--Y0mTL"><figure class="content-image__figure--7vume"><picture><img class="progressive-image-base__fade--Nrn20 progressive-image-base__fade--is-ready--dMxKu content-image__image--7tGlg" loading="eager" src="${user_data.avatar}" alt="User avatar."></picture></figure></div>`;
  ...
  const speed_up = (point) => {
    ...
    document
      ...
      .contentWindow.postMessage({ cmd: "speed_to", time: point }, "*");
  };
  ...
  async function runSystem() {
    ...
    const [ep_id, ep_name] = url.pathname
      .split("/")
      .slice(1)
      .filter((n, i, all) => {
        if (n == "watch") {
    ...
    const userId =
      ...
      Object.keys(localStorage).find((e) => e.startsWith("ab.storage.userId"))
        ? localStorage.ajs_user_id
          ? JSON.parse(localStorage.ajs_user_id)
          : JSON.parse(
              localStorage[
                Object.keys(localStorage).find((e) =>
                  e.startsWith("ab.storage.userId"),
                )
              ],
            ).v.g
        : null; // user id will be used to identify user on comments
    console.log(`Using userid: ${userId}`);
    function getUserLoggedInDetails() {
      if (!userId) return null;
      const p = document.getElementsByClassName(
        "erc-authenticated-user-menu",
      )[0];
      return {
        avatar:
          p.children[0].children[0].children[0].children[0].children[0]
            .children[0].children[0].children[0].src,
        name: p.children[1].children[0].children[0].querySelector("h4")
          .innerHTML,
      };
    }
    const ep_data = await fetch(
      `https://api.saahild.com/api/crunchyroll/comments/${ep_id}/${ep_name}`,
      {
        headers: {
          "x-user-id": userId,
        },
    ...
    function createSubmitPage(parent) {
      ...
      h3.className =
        "heading--nKNOf heading--is-xs--UyvXH heading--is-family-type-one--GqBzU list-info-box__title--G6GzN";
      h3.innerText = "Write Comment";
      textarea.style.marginBottom = "2.5rem";
      submitButton.className =
        "button--xqVd0 button--is-type-one-weak--KLvCX load-more-button";
      textarea.rows = "30";
      textarea.cols = "30";
      textarea.className = "comment-textarea__editable-area--gKhpt";
      submitButton.innerHTML =
        '<span class="call-to-action--PEidl call-to-action--is-m--RVdkI button__cta--LOqDH">Submit</span>';
    ...
    function createCommentEl(payload = EXAMPLE_MESSAGE_PAYLOAD_GEN()) {
      ...
      if (!badges) badges = [];
      if (payload.updated_at && payload.created_at !== payload.updated_at)
        badges = [{ name: "EDITED" }, ...badges];
      console.log(payload);
      ...
      likeBtn.disabled = !userId;
      likeEl.className = "comment-actions__item-wrapper--CbVBD";
      likeBtn.className =
        "call-to-action--PEidl call-to-action--is-s--xFu35 comment-actions__item--5xkC3";
      ...
      likeBtn.onclick = () => {
        // fetch('https:/')
        fetch(
          `https://api.saahild.com/api/crunchyroll/comments/${ep_id}/${ep_name}/${id || created_at}/like`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
<path d="M 20.5 4 A 1.50015 1.50015 0 0 0 19.066406 6 L 14.640625 6 C 12.803372 6 11.082924 6.9194511 10.064453 8.4492188 L 7.6972656 12 L 7.5 12 A 1.50015 1.50015 0 1 0 7.5 15 L 8.2636719 15 A 1.50015 1.50015 0 0 0 8.6523438 15.007812 L 11.125 38.085938 C 11.423352 40.868277 13.795836 43 16.59375 43 L 31.404297 43 C 34.202211 43 36.574695 40.868277 36.873047 38.085938 L 39.347656 15.007812 A 1.50015 1.50015 0 0 0 39.728516 15 L 40.5 15 A 1.50015 1.50015 0 1 0 40.5 12 L 40.302734 12 L 37.935547 8.4492188 C 36.916254 6.9202798 35.196001 6 33.359375 6 L 28.933594 6 A 1.50015 1.50015 0 0 0 27.5 4 L 20.5 4 z M 14.640625 9 L 33.359375 9 C 34.196749 9 34.974746 9.4162203 35.439453 10.113281 L 36.697266 12 L 11.302734 12 L 12.560547 10.113281 A 1.50015 1.50015 0 0 0 12.5625 10.111328 C 13.025982 9.4151428 13.801878 9 14.640625 9 z M 11.669922 15 L 36.330078 15 L 33.890625 37.765625 C 33.752977 39.049286 32.694383 40 31.404297 40 L 16.59375 40 C 15.303664 40 14.247023 39.049286 14.109375 37.765625 L 11.669922 15 z"></path>
      ...
      dislikeBtn.onclick = () => {
        // fetch('https:/')
        fetch(
          `https://api.saahild.com/api/crunchyroll/comments/${ep_id}/${ep_name}/${id || created_at}/dislike`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      editBtn.onclick = () => {
        ...
        if (commentBody.children[0].querySelector("textarea")?.value) {
          let newContent =
            commentBody.children[0].querySelector("textarea")?.value;
          console.log(newContent);
          fetch(
            `https://api.saahild.com/api/crunchyroll/comments/${ep_id}/${ep_name}/${id || created_at}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      commentDate.className =
        "text--gq6o- text--is-m--pqiL- comment__publication-date--1Enil";
      try {
        commentDate.innerText = dayjs().to(dayjs(created_at));
      } catch (e) {
        // dayjs broken on firefox
        commentDate.innerText =
          window.ms(Date.now() - created_at, true) + " ago";
      }
    ...
    commentsCount.className =
      "heading--nKNOf heading--is-xxs--1CKSn heading--is-family-type-one--GqBzU comments-count";
    commentsCount.innerText = "{comments_count} Comments".replace(
      "{comments_count}",
      ep_data.length,
    );
    ...
    if (ep_data.length == 0) {
      ...
      const safeModeQ = await instance.storage.sync
        .get("safeMode")
        .then((v) => v.safeMode);
      ...
      if (safeModeQ) {
        ...
        await Promise.all(
          ep_data.map(async (e) => {
            return {
              ...e,
              safeMode: !(await instance.runtime.sendMessage({
                cmd: "check_content",
                msg: e.content,
              })),
            };
  ...
  setInterval(() => {
    ...
    if (
      document.getElementsByClassName("commenting-wrapper")[0] &&
      urlWasChanged
    ) {
